...
| --- | --- | --- | --- | --- | --- |
| `fast-xml-parser` (transitive via AWS SDK) | AWS SDK XML building internals (`@aws-sdk/client-s3` chain) | High (`GHSA-jmr7-xgp7-cmfj`) | Mitigated with `pnpm` override to `5.3.6`; verify via `pnpm list fast-xml-parser --depth 12` and `pnpm audit --prod` | Eng | 0 |
...
- `pnpm audit --prod` -> `1 moderate` (high `fast-xml-parser` finding resolved).
- `pnpm list fast-xml-parser --depth 12` -> AWS SDK chain now resolves to `fast-xml-parser@5.3.6`.
