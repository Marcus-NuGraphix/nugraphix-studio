...
| T-007 | P0 | 0 | Resolve high dependency vulnerability (`fast-xml-parser` chain) | Eng | Todo |
...
| T-010 | P0 | 1 | Reconcile migration artifacts so `db:migrate` fully boots schema for seed flow | Eng | Todo |

## In Progress

| ID | Phase | Task | Notes |
| --- | --- | --- | --- |
| T-008 | 1 | Implement deterministic local bootstrap seed path (admin + content) | Local seed works after `db:push`; migration-only path still fails due missing `user` table on clean DB. |
